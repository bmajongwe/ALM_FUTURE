...
def classify_and_store_hqla_multi_ccy(fic_mis_date):
    ...
    - Summary rows are flagged with is_total=True and a total_type.
    """
    ...
    try:
        ...
        with transaction.atomic():
            ...
            for (prod_type, prod_code, ccy), original_sum_amt in grouped_data.items():
                ...
                # Line-level detail for each aggregated group (non-total rows)
            ...
            for ccy, entries in currency_to_hqla_entries.items():
                ...
                # a) "Before Cap": apply haircuts to Level 2A and 2B (Level 1 is not subject to haircut)
                ...
                # Summary rows are flagged as totals (is_total=True) with an appropriate total_type.

                # "Before Caps" total (overall) now labeled as overall_before
                ...
                # "After Caps" total (overall) now labeled as overall_after
                all_insertable_rows.append(HQLAStock(
                    fic_mis_date=fic_mis_date,
                ...
                # Finally, add the line-by-line product detail entries (non-total rows)
