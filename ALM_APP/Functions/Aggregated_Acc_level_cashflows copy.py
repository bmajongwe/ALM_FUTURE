...
def filter_products(process_name, fic_mis_date):
    ...
    try:
        ...
        logger.info(f"Filtered product codes: {list(final_product_codes)}")
...
def calculate_time_buckets_and_spread(process_name, fic_mis_date):
    ...
    try:
        ...
        logger.info(f"Reset 'record_count' for {reset_count} rows in product_level_cashflows for fic_mis_date {fic_mis_date}")
        ...
        logger.info(f"Updated 'record_count' for {updated_rows} rows in product_level_cashflows for fic_mis_date {fic_mis_date}")
        ...
        logger.info(f"Deleted existing records for process '{process_name}' and fic_mis_date {fic_mis_date}")
        ...
        if not time_buckets.exists():
            logger.warning("No time buckets found.")
            return 0
        ...
        if not cashflows.exists():
            logger.info(f"No cashflows found for fic_mis_date: {fic_mis_date}")
            return 0
        ...
        logger.info(f"Successfully aggregated cashflows for process '{process_name}' and fic_mis_date {fic_mis_date}.")
        return 1

    except Exception as e:
        logger.error(f"Error in calculate_time_buckets_and_spread: {e}")
        logger.debug(traceback.format_exc())
        return 0
...
def calculate_behavioral_pattern_distribution(process_name, fic_mis_date):
    ...
    try:
        ...
        if not behavioral_patterns.exists():
            logger.warning("No behavioral patterns found.")
            return 0
        ...
        for pattern in behavioral_patterns:
            ...
            if total_entries != total_time_buckets:
                logger.warning(f"Mismatch between behavioral entries ({total_entries}) and time buckets ({total_time_buckets}). Skipping pattern '{pattern.v_prod_type}'.")
            ...
            if not cashflows.exists():
                logger.info(f"No cashflows found for fic_mis_date: {fic_mis_date} and product type: {pattern.v_prod_type}")
                continue
        ...
        logger.info(f"Successfully distributed amounts using behavioral patterns for process '{process_name}' and fic_mis_date {fic_mis_date}.")
        return 1

    except Exception as e:
        logger.error(f"Error in calculate_behavioral_pattern_distribution: {e}")
        logger.debug(traceback.format_exc())
        return 0
