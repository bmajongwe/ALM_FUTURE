...
def calculate_cash_flows_for_instrument(instrument):
    try:
        ...
        error_message = f"Error calculating cash flows for instrument {instrument.v_account_number}: {str(e)}"
        print(error_message)
        Log.objects.create(
...
def project_cash_flows(fic_mis_date):
    try:
        deleted_count = FSI_Expected_Cashflow.objects.filter(fic_mis_date=fic_mis_date).delete()[0]
        logger_message = f"Deleted {deleted_count} existing cashflows for fic_mis_date: {fic_mis_date}"
        ...
        if not instruments.exists():
            logger_message = f"No loans found for the given fic_mis_date: {fic_mis_date}"
            print(logger_message)
            Log.objects.create(
                function_name='project_cash_flows',
                log_level='WARNING',
        ...
        logger_message = f"Total of {total_cash_flows} cash flows projected for {instruments.count()} loans for MIS date {fic_mis_date}."
        print(logger_message)
        Log.objects.create(
            function_name='project_cash_flows',
            log_level='INFO',
        ...
        error_message = f"Error projecting cash flows for fic_mis_date {fic_mis_date}: {str(e)}"
        print(error_message)
        Log.objects.create(
            function_name='project_cash_flows',
            log_level='ERROR',
