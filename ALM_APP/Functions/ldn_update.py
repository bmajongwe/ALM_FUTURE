...
from ..models import Ldn_Financial_Instrument, Dim_Product, Log
import traceback
...
def update_date(fic_mis_date):
    ...
    try:
        for bucket, days in duration_mapping.items():
            logger_message = f"Processing bucket '{bucket}' with duration {days} days."
            ...
            if not eligible_prod_codes.exists():
                logger_message = f"No eligible product codes found for bucket '{bucket}'."
                print(logger_message)
                Log.objects.create(
                    function_name='update_date',
                    log_level='WARNING',
            ...
            record_count = matching_records.count()
            logger_message = f"Found {record_count} matching records for bucket '{bucket}' and fic_mis_date {fic_mis_date}."
            print(logger_message)
            Log.objects.create(
                function_name='update_date',
                log_level='INFO',
            ...
            # Update D_MATURITY_DATE and D_NEXT_PAYMENT_DATE
            updated_records = matching_records.update(
                d_maturity_date=F('fic_mis_date') + timedelta(days=days),
                d_next_payment_date=F('fic_mis_date') + timedelta(days=days - 1)
            )

            logger_message = f"Updated {updated_records} records for bucket '{bucket}' and fic_mis_date {fic_mis_date}."
            print(logger_message)
            Log.objects.create(
                function_name='update_date',
                log_level='INFO',
        ...
        logger_message = f"Successfully updated dates for fic_mis_date {fic_mis_date}."
        print(logger_message)
        Log.objects.create(
            function_name='update_date',
            log_level='INFO',
        ...
        error_message = f"Error updating dates for fic_mis_date {fic_mis_date}: {str(e)}"
        print(error_message)
        Log.objects.create(
            function_name='update_date',
            log_level='ERROR',
