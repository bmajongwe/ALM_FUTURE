...
from ..models import TimeBuckets, Dim_Dates, Log
import calendar
import traceback
...
def populate_dim_dates_from_time_buckets(fic_mis_date):
    ...
    try:
        ...
        updated_count = Dim_Dates.objects.filter(fic_mis_date=previous_fic_mis_date).update(f_latest_record_indicator='N')
        logger_message = f"Updated {updated_count} records to 'N' for previous fic_mis_date: {previous_fic_mis_date}"
        print(logger_message)
        Log.objects.create(
            function_name='populate_dim_dates_from_time_buckets',
            log_level='INFO',
        ...
        deleted_count = Dim_Dates.objects.filter(fic_mis_date=fic_mis_date).delete()[0]
        ...
        for time_bucket in time_buckets:
            ...
            # Log each bucket insertion
            logger_message = f"Inserted/Updated bucket {time_bucket.serial_number} for fic_mis_date {fic_mis_date}"
            print(logger_message)
            Log.objects.create(
                function_name='populate_dim_dates_from_time_buckets',
                log_level='INFO',
        ...
        logger_message = f"Successfully populated Dim_Dates for fic_mis_date {fic_mis_date}."
        print(logger_message)
        Log.objects.create(
            function_name='populate_dim_dates_from_time_buckets',
            log_level='INFO',
        ...
        error_message = f"Error populating Dim_Dates for fic_mis_date {fic_mis_date}: {str(e)}"
        print(error_message)
        Log.objects.create(
            function_name='populate_dim_dates_from_time_buckets',
            log_level='ERROR',
